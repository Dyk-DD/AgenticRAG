    ...
    with st.chat_message("user"):
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                memory_context = ""
                if rag.memory_module and rag.current_session_id:
                    memory_context = rag.memory_module.retrieve_memory_context(prompt, rag.current_session_id)
                ...
                # 3. 调用重构后的生成模块
                if enable_stream:
                    response_generator = rag.generation_module.generate_adaptive_answer_stream(prompt, relevant_docs, memory_context)
                    ...
                    full_response = rag.generation_module.generate_adaptive_answer(prompt, relevant_docs, memory_context)
                    message_placeholder.markdown(full_response)

                # 4. 记录本轮到记忆
                if rag.memory_module and rag.current_session_id:
                    extracted = rag.memory_module.extract_referenced_entities(prompt, full_response)
